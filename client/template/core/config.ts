...
class ConfigManager {
  ...
  public initialize(config: QuizConfig) {
    if (this.config) {
  ...
  private createDefaultConfig(): QuizConfig {
  ...
  public updateConfig(updates: Partial<QuizConfig>) {
    const config = this.getConfig();
...
export const createConfig = (options: CreateConfigOptions = {}): QuizConfig => {
  let authConfig;
